...
export const removeBackground = async (imageElement: HTMLImageElement): Promise<Blob> => {
  try {
    ...
    const imageData = canvas.toDataURL('image/jpeg', 0.8); // Using JPEG for model input
    ...
    outputCtx.drawImage(canvas, 0, 0); // Draw original (potentially resized) image
    
    const outputImageData = outputCtx.getImageData(0, 0, outputCanvas.width, outputCanvas.height);
    const data = outputImageData.data;
    
    for (let i = 0; i < result[0].mask.data.length; i++) {
      const alpha = Math.round((1 - result[0].mask.data[i]) * 255); // Invert mask for subject
      data[i * 4 + 3] = alpha;
    }
    ...
    return new Promise((resolve, reject) => {
      outputCanvas.toBlob(
        (blob) => {
          if (blob) {
            console.log('Successfully created final blob (PNG)');
            resolve(blob);
          } else {
